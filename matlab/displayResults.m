...
function displayResults(vPlayer, frame, trk, RoI, tConfidence, tAge)

if ~isempty(trk)
    confidence = reshape([trk(:).confidence], 2, [])';
    maxConfidence = confidence(:, 1); avgConfidence = confidence(:, 2);
...
frame = insertShape(frame, 'Rectangle', RoI, 'Color',[255, 0, 0], 'LineWidth',1);
